...
const SECTIONS = [
  { title: '🧠 KYLE RAWSON SALES BRAIN | Who Kyle Is', color: '#5865F2',
    body: '**3 Roles:**
- Closer for **AIPA** (Kyran mentorship) $4,800/6mo
...
**Cash collected: $1M+** | Contractor niche specialist' },
  { title: '💰 Recent Closes March/Feb 2026', color: '#57F287',
    body: '**SUMMIT BOOST:**
...
- Rakesh & Siddeshwar $4,800/6mo at $800/mo' },
  { title: '🎯 Core Sales Framework', color: '#FEE75C',
    body: '**THE FLOW:**
...
High revenue = Logical: systems ROI scalability
Smaller prospect = Emotional: immediate wins quick transformation' },
  { title: '🔥 Objection Handling', color: '#ED4245',
    body: '**Too expensive** - Break it down: $1,500 service + $750 ad spend. Never drop price first
...
**Bad past experience** - Tell me exactly what happened, then show what is different' },
  { title: '💡 Pricing Tactics', color: '#EB459E',
    body: '- Quote full price upfront on setting calls $2-3k/mo to filter tire-kickers
...
- Guarantee: X leads/month or work free until hit' },
  { title: '📋 Setting Call Rules + KPI Targets', color: '#4285F4',
    body: '- Pre-qualify budget on setting call, mention $2k/mo upfront
...
- Benchmark: 26 booked / 50% show / 38% close = $5k/week' },
  { title: '👥 3AM Closing Client Intel', color: '#FF6B35',
    body: '**Strive Online (Ishaan):** Joao closes, Helena expanding to closing. 2-day booking rule.
**InstantAppointment AI (Pranshu):** Younes active. Caden PAUSED - 0 from 10+ calls.
**Simplifi (Matt & Anthony):** Anthony closes. $1,500 + $500/close offer. Setter Furqan trialing.
**MountLeader Media (Rob):** Tilesh closes. Underperforming - monitor.' },
  { title: '🚀 Kyle Core Principles', color: '#9B59B6',
    body: '1. It needs to make sense - never force a deal
...
async function postSalesBrain(client) {
  ...
  try {
    ...
    if (!cn) { console.log('channel not found'); return; }
...
---');
    for (const s of SECTIONS) {
      const embed = new EmbedBuilder().setTitle(s.title).setColor(s.color).setDescription(s.body).setFooter({ text: 'Jarvis Sales Brain | Built from Kyle raw calls' });
      await cn.send({ embeds: [embed] });
      await new Promise(r => setTimeout(r, 600));
    }
    const sb = client.channels.cache.get(SALES_BRAIN_ID);
    if (sb) {
      const embed = new EmbedBuilder().setTitle('Sales Brain is Loaded').setColor('#5865F2')
...
Use /ask in any channel')
  ...
  } catch(e) { console.error('Sales brain post error:', e.message); }
}

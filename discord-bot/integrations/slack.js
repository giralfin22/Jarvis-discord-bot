        ...
        try {
          ...
          for (const msg of (history.messages || [])) {
            ...
            if (this.urgentKeywords.some(kw => (msg.text || '').toLowerCase().includes(kw))) {
              ...
              try { const u = await this.client.users.info({ user: msg.user }); userName = u.user?.real_name || 'Unknown'; } catch (_) {}
              urgentMessages.push({ text: msg.text, user: userName, channel: channel.name, ts: msg.ts });
            }
          }
        } catch (e) {}
      }
